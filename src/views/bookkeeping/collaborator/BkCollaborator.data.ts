...
export const columns: BasicColumn[] = [
  ...
  },
  {
    ...
    customRender: ({ record }) => {
      ...
      const status = ~~collaboratorStatus === 0;
...
export const formSchema: FormSchema[] = [
  {
    ...
    dynamicRules: ({ model, schema }) => {
      return [{ required: true, message: '请输入厂商/个体 公司名称!' }];
    },
  },
  {
    label: '联系方式',
    ...
    dynamicRules: ({ model, schema }) => {
      return [{ required: true, message: '请输入厂商/个体 公司固话/手机号码!' }];
    },
  },
  {
    label: '合作状态',
    ...
    component: 'InputNumber',
    dynamicRules: ({ model, schema }) => {
      return [{ required: true, message: '请输入是否还在合作，0为正常，1为不合作!' }];
    },
  },
  {
  ...
  },
  {
    ...
    dynamicRules: ({ model, schema }) => {
      return [
        {
          required: true,
          message: '请输入商户状态，0为只采购不销售（也就是客户），1为只销售不采购（也就是供货商），2为合作同行（既是客户又供货商）!',
        },
      ];
    },
  },
  {
    ...
    component: 'InputNumber',
    dynamicRules: ({ model, schema }) => {
      return [{ required: true, message: '请输入默认匿名账户为0（有且只有一个）个体为 1  厂商为 2!' }];
    },
  },
  // TODO 主键隐藏字段，目前写死为ID
  {
