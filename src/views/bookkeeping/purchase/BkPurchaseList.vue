<template>
  <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection">
      ...
      <template #unitPrice="{text}">
        <a-tag color="green">{{text}}￥</a-tag>
      </template>
      <template #allPrice="{text}">
        <a-tag color="red">{{text}}￥</a-tag>
      </template>
      <template #purchaseReceipt="{text}">
        <a-tag :color="text=='增值税'?'red':'green'">{{text}}</a-tag>
      </template>
    </BasicTable>
    <!-- 表单区域 -->
    <BkPurchaseModal @register="registerModal" @success="handleSuccess"></BkPurchaseModal>
  </div>
</template>

<script lang="ts" name="bookkeeping-bkPurchase" setup>
